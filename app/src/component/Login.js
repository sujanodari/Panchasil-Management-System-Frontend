...
class Login extends Component {
  ...
  componentDidMount() {
    //for getting news
    axois
      .get("http://localhost:3012/api/v1/news", this.state.config)
      .then((response) => {
        this.setState({
          news: response.data,
        });
  ...
  login = (e) => {
    ...
    if (this.state.email === 0 || emailIsValid(this.state.email) === false) {
      this.setState({
        Verror: true,
      });
    } else if (
      ...
      axois
        .post("http://localhost:3012/api/v1/users/signin", this.state)
        .then((response) => {
            ...
            <div className="row">
              <div className="col-md-12">
                ...
                {this.state.Lstatus === "Not found" ? (
                  <p className="labelColor">User not found !!!</p>
                ) : null}
                {this.state.Lstatus === "Password" ? (
                  <p className="labelColor">Password don't match !!!</p>
                ) : null}
                {this.state.Lstatus === "Verify" ? (
                  <p className="labelColor">User not verified!!!</p>
                ) : null}
                ...
                <Form>
                  ...
                  <Form.Group controlId="formBasicPassword">
                    ...
                    {this.state.errors.password ? (
                      <label className="labelColor" name="errPassword">
                        {this.state.errors.password}
                      </label>
                    ) : null}
                  </Form.Group>
