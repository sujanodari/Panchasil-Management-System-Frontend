...
class AdminDashboard extends Component {
  ...
  componentDidMount() {
    ...
    axois
      .get("http://localhost:3012/api/v1/approve", this.state.config)
      .then((response) => {
        this.setState({
          users: response.data,
        });
  ...
  handleChange = (e) => {
    ...
    if (e.target.value === "Student") {
      ...
      axois
        .get("http://localhost:3012/api/v1/users/student", this.state.config)
        .then((response) => {
          this.setState({
            allUsers: response.data,
          });
      ...
      //for getting all student
      axois
        .get("http://localhost:3012/api/v1/users/staff", this.state.config)
        .then((response) => {
  ...
  searchByName = (e) => {
    ...
    axois
      .get("http://localhost:3012/api/v1/search", this.state.config, this.state)
      .then((response) => {
        var x = [];
    ...
    return (
      <>
        <div>
          ...
          <div>
            <p align="center" className="admin">
              <label id="welcome"> Welcome</label>: <b>{this.state.fullname}</b>{" "}
            </p>
            {this.state.auth === false ? (
              <p align="center" className="labelColor">
                <label> User is not authorised to verify</label>{" "}
              </p>
            ) : null}
          </div>
                ...
                <Card.Body>
                  ...
                  <Card.Text>
                    {this.state.users.length === 0 ? (
                      <p align="center">All users are approved</p>
                    ) : (
                      <Approve
