...
class ViewClass extends Component {
  ...
  handleDelete(id) {
    axois
      .delete(`http://localhost:3012/api/v1/class/${id}`, this.state.config)
      .then((response) => {
        axois
          .get("http://localhost:3012/api/v1/class", this.state.config)
          .then((response) => {
  ...
  handleDeleteRoutine = (id) => {
    axois
      .delete("http://localhost:3012/api/v1/routine/" + id, this.state.config)
    ...
    return (
      <>
        ...
        <div className="container">
          <p align="center">
            <b>No of class: {this.state.allClass.length}</b>{" "}
          </p>
          <hr />
          {this.props.location.state ? (
            <label className="labelColor">{this.props.location.state}</label>
          ) : null}
          {this.state.allClass.length === 0 ? (
                    ...
                    <Card.Body>
                      ...
                      <Card.Text>
                        ...
                        {allClass.routine ? (
                          <>
                            <a
                              href={`http://localhost:3012/images/${allClass.routine}`}
                            >
                              <Card.Img
                                variant="top"
                                src={`http://localhost:3012/images/${allClass.routine}`}
                                className="fix-image"
                                alt="image not found"
                              />
                            </a>
