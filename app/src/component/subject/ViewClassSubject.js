...
componentDidMount(){
    ...
    axios.get('http://localhost:3012/api/v1/subjectsClass', this.state.config)
    .then((response=>{
      this.setState({
...
handleChange = e => {
    ...
    delete(id){
        axios.delete('http://localhost:3012/api/v1/subjectsClass/'+id,this.state.config)
        .then(response=>{
           this.setState({
               Verror:false,
                success:true,
        ...
        <div>
           ...
           <div className="container fixed"> 
                <h3 align="center">Subject for Class</h3>
                     {this.state.success===true ? <label className="labelColor" name="errEmail">subject for class is deleted</label>
                     : null}
...

                     {
                       ...
                       <Table striped bordered hover variant="dark">
                        ...
                        {  this.state.subjectsClass.map((subjectsClass)=>
                            <tr value={ this.state.subclassId=subjectsClass.subclassId,
                            ...
                            <td> {this.state.class} {this.state.section}</td>
                            <td> {this.state.subject}</td>
                            <td><Button variant="success" onClick={()=> this.delete(subjectsClass.subclassId)} type="submit" className="btn-block">
                                 Delete
                                 </Button>
                          </td>
                            </tr>
